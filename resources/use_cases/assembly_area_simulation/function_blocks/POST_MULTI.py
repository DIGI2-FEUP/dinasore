...
class POST_MULTI:
    ...
    def writeFile(self):
        ...
        the_file.write(
            "{0},{1},{2},{3}\n".format(threading.currentThread().getName(), self.material_name, self.container_name,
                                       datetime.now()))
        the_file.close()

    def schedule(self, event_name, event_value, material_name, container_name, post_id, main_post_id, receive_list,
                 fault_prob):

        if event_name == 'INIT':
            if receive_list is not None and len(receive_list) > 0:
                self.receive_list = [0] * len(receive_list)
            ...
            # End State
                ...
                elif self.receive_list is not None and not all(i >= 1 for i in self.receive_list):
                    return [None, None, None, None, None, None,
                            None, None,
                            self.states_sequence[self.actual_state]]
                else:
                ...
                # decrement counters before send
                if receive_list is not None and len(receive_list) > 0:
                    self.receive_list = [x - 1 for x in self.receive_list]

                # return [None, None, self.sync, None, None, self.event,
                #     self.material_name, self.container_name,
                #     self.states_sequence[self.actual_state]]
                if post_id is not None:
                    return [None, None, self.sync, None, None, post_id,
                            self.material_name, self.container_name,
                            self.states_sequence[self.actual_state]]
                else:
                    return [None, None, self.sync, None, None, self.event,
        ...
        # standby
            ...
            if receive_list is not None and len(receive_list) > 0 and len(self.receive_list) < 1:
                self.receive_list = [0] * len(receive_list)

            # update receive list if not empty
            if receive_list is not None and len(receive_list) > 0 and main_post_id is not None and \
                    main_post_id in receive_list and event_value in receive_list:
                # update correct position within the list
                index = receive_list.index(event_value)
                self.receive_list[index] += 1

            # updates the state

            if self.actual_state != 0:
                if self.receive_list is not None and len(self.receive_list) > 0 and len(receive_list) > 0:
                    self.actual_state = 1
                    return [None, 1, None, None, None, None,
                            None, None,
                            self.states_sequence[self.actual_state]]
